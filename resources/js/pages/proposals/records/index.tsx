...
export default function ProposalRecords({ auth, proposals, categories = [], filters = {} }: Props) {
    const [selectedCategory, setSelectedCategory] = useState<string>(filters.category || 'all');
    const [currentPage, setCurrentPage] = useState(proposals.current_page);
    ...
    const debouncedSearch = _.debounce((value: string) => {
        ...
        router.get(route('proposals.records.index'), {
            search: value,
    ...
    // Helper function to format date in numeric format (MM/DD/YYYY)
                        ...
                        <CardContent className="space-y-6">
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        ...
                                        <TableHead>Implementation Period</TableHead>
                                ...
                                <TableBody>
                                    {proposalsList.map((proposal) => (
                                        <TableRow key={proposal.id}>
                                            ...
                                            <TableCell>{formatBudget(proposal.estimated_cost || proposal.total_budget)}</TableCell>
                                            <TableCell>
                                                <div className="flex flex-col space-y-2">
                                                    <div className="flex items-center">
                                                        <Calendar className="mr-2 h-4 w-4 text-gray-500" />
                                                        <span className="text-sm">Start: {formatNumericDate(proposal.implementation_start_date)}</span>
                                                    </div>
                                                    <div className="flex items-center">
                                                        <Calendar className="mr-2 h-4 w-4 text-gray-500" />
                                                        <span className="text-sm">End: {formatNumericDate(proposal.implementation_end_date)}</span>
                                                    </div>
                                                </div>
                                            </TableCell>
                                            <TableCell>
                                                <Link href={route('proposals.records.show', proposal.id)}>
                            ...
                            {proposalsList.length === 0 && (
                                <div className="text-center py-12">
                                    <p className="text-gray-500">No approved proposals found.</p>
                                </div>
                            )}
